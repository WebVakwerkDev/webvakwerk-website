...
const AanvraagPage = () => {
  ...
  const subjectAutoGenerated = useRef(true);

  const progress = useMemo(() => ((currentStep + 1) / steps.length) * 100, [currentStep]);

  useEffect(() => {
    if (!subjectAutoGenerated.current) return;
    const suggestion = generateSubjectSuggestion(payload.serviceType, payload.websiteType);
    if (suggestion) {
      setPayload((current) => ({ ...current, subject: suggestion }));
    }
  }, [payload.serviceType, payload.websiteType]);
                  ...
                  <div className="grid gap-5 sm:grid-cols-2">
                    ...
                    >
                      <Input
                        ...
                        onChange={(event) => {
                          subjectAutoGenerated.current = false;
